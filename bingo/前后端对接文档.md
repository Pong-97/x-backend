# 前后端对接文档

## 📋 文档说明

本文档用于前后端开发对接，包含完整的 API 接口说明、数据结构、认证方式、错误处理等信息。

**后端项目信息**：
- 技术栈：Node.js + Express + MongoDB
- 部署地址：`https://lyaftwgtwyqr.sealosbja.site`
- 本地开发：`http://localhost:3000`

---

## 🔧 技术规范

### 1. 接口协议
- 协议：HTTP/HTTPS
- 数据格式：JSON
- 字符编码：UTF-8
- API 风格：RESTful

### 2. 请求头规范

#### 公共请求头
```http
Content-Type: application/json
```

#### 需要认证的接口
```http
Content-Type: application/json
Authorization: Bearer <token>
```

### 3. 统一响应格式

#### 成功响应
```json
{
  "code": 200,
  "message": "success",
  "data": {
    // 具体业务数据
  }
}
```

#### 失败响应
```json
{
  "code": 400,
  "message": "错误描述信息",
  "data": null
}
```

### 4. 状态码说明

| 状态码 | 说明 |
|--------|------|
| 200 | 请求成功 |
| 400 | 请求参数错误 |
| 401 | 未授权，token 无效或过期 |
| 403 | 无权限访问 |
| 404 | 资源不存在 |
| 500 | 服务器内部错误 |

---

## 🔐 认证机制

### JWT Token 认证

1. **获取 Token**：用户登录成功后，后端返回 JWT token
2. **使用 Token**：前端将 token 存储（localStorage/sessionStorage），在需要认证的接口请求头中携带
3. **Token 格式**：`Authorization: Bearer <token>`
4. **Token 有效期**：7 天
5. **Token 刷新**：token 过期后需要重新登录

### 前端示例代码

```javascript
// axios 拦截器配置
axios.interceptors.request.use(config => {
  const token = localStorage.getItem('token');
  if (token) {
    config.headers.Authorization = `Bearer ${token}`;
  }
  return config;
});

// 响应拦截器处理 401
axios.interceptors.response.use(
  response => response,
  error => {
    if (error.response?.status === 401) {
      // 清除 token，跳转到登录页
      localStorage.removeItem('token');
      router.push('/login');
    }
    return Promise.reject(error);
  }
);
```

---

## 📡 API 接口清单

### 接口概览

| 模块 | 接口数量 | 是否需要认证 |
|------|---------|-------------|
| 用户模块 | 4 | 部分需要 |
| 商品模块 | 3 | 不需要 |
| 分类模块 | 1 | 不需要 |
| 购物车模块 | 4 | 需要 |
| 订单模块 | 6 | 需要 |
| 地址模块 | 5 | 需要 |
| 首页模块 | 1 | 不需要 |

---

## 1️⃣ 用户模块

### 1.1 用户注册

**接口地址**：`POST /user/register`

**是否需要认证**：否

**请求参数**：
```json
{
  "username": "testuser",      // 必填，3-20字符
  "password": "123456",        // 必填，6-20字符
  "email": "test@example.com"  // 选填，邮箱格式
}
```

**响应示例**：
```json
{
  "code": 200,
  "message": "注册成功",
  "data": {
    "userId": 1001
  }
}
```

**错误情况**：
- `400`：用户名已存在
- `400`：邮箱已被使用
- `400`：参数格式错误

---

### 1.2 用户登录

**接口地址**：`POST /user/login`

**是否需要认证**：否

**请求参数**：
```json
{
  "username": "testuser",  // 必填
  "password": "123456"     // 必填
}
```

**响应示例**：
```json
{
  "code": 200,
  "message": "登录成功",
  "data": {
    "token": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9...",
    "userInfo": {
      "userId": 1001,
      "username": "testuser",
      "email": "test@example.com",
      "phone": "13800138000",
      "avatar": "https://example.com/avatar.jpg"
    }
  }
}
```

**前端处理**：
```javascript
// 登录成功后保存 token
const { token, userInfo } = response.data.data;
localStorage.setItem('token', token);
localStorage.setItem('userInfo', JSON.stringify(userInfo));
```

---

### 1.3 获取用户信息

**接口地址**：`GET /user/info`

**是否需要认证**：是

**请求参数**：无

**响应示例**：
```json
{
  "code": 200,
  "message": "success",
  "data": {
    "userId": 1001,
    "username": "testuser",
    "email": "test@example.com",
    "phone": "13800138000",
    "avatar": "https://example.com/avatar.jpg",
    "createdAt": "2024-01-01T00:00:00.000Z"
  }
}
```

---

### 1.4 更新用户信息

**接口地址**：`POST /user/update`

**是否需要认证**：是

**请求参数**：
```json
{
  "email": "newemail@example.com",  // 选填
  "phone": "13900139000",           // 选填
  "avatar": "https://new-avatar.jpg" // 选填
}
```

**响应示例**：
```json
{
  "code": 200,
  "message": "更新成功",
  "data": null
}
```

---

## 2️⃣ 商品模块

### 2.1 获取商品列表

**接口地址**：`GET /product/list`

**是否需要认证**：否

**请求参数**（Query）：
```
page=1              // 页码，默认1
pageSize=10         // 每页数量，默认10
categoryId=1        // 分类ID，选填
sortBy=sales        // 排序字段：sales(销量)、price(价格)、createdAt(时间)
order=desc          // 排序方式：asc(升序)、desc(降序)
```

**完整示例**：
```
GET /product/list?page=1&pageSize=10&categoryId=1&sortBy=sales&order=desc
```

**响应示例**：
```json
{
  "code": 200,
  "message": "success",
  "data": {
    "list": [
      {
        "productId": 1001,
        "name": "iPhone 15 Pro",
        "price": 7999,
        "originalPrice": 8999,
        "image": "https://example.com/iphone.jpg",
        "description": "最新款iPhone",
        "stock": 100,
        "sales": 520,
        "categoryId": 1,
        "status": 1
      }
    ],
    "total": 50,
    "page": 1,
    "pageSize": 10,
    "totalPages": 5
  }
}
```

---

### 2.2 获取商品详情

**接口地址**：`GET /product/:id`

**是否需要认证**：否

**请求参数**：
- `id`：商品ID（路径参数）

**示例**：
```
GET /product/1001
```

**响应示例**：
```json
{
  "code": 200,
  "message": "success",
  "data": {
    "productId": 1001,
    "name": "iPhone 15 Pro",
    "price": 7999,
    "originalPrice": 8999,
    "image": "https://example.com/iphone.jpg",
    "images": [
      "https://example.com/iphone1.jpg",
      "https://example.com/iphone2.jpg"
    ],
    "description": "最新款iPhone",
    "detail": "<p>详细描述HTML内容</p>",
    "stock": 100,
    "sales": 520,
    "views": 1234,
    "categoryId": 1,
    "specs": [
      { "name": "颜色", "value": "深空黑" },
      { "name": "容量", "value": "256GB" }
    ],
    "status": 1
  }
}
```

---

### 2.3 搜索商品

**接口地址**：`GET /product/search`

**是否需要认证**：否

**请求参数**（Query）：
```
keyword=iPhone      // 搜索关键词，必填
page=1             // 页码，默认1
pageSize=10        // 每页数量，默认10
```

**示例**：
```
GET /product/search?keyword=iPhone&page=1&pageSize=10
```

**响应示例**：
```json
{
  "code": 200,
  "message": "success",
  "data": {
    "list": [...],
    "total": 15,
    "page": 1,
    "pageSize": 10
  }
}
```

---

## 3️⃣ 分类模块

### 3.1 获取分类列表

**接口地址**：`GET /category/list`

**是否需要认证**：否

**请求参数**：无

**响应示例**：
```json
{
  "code": 200,
  "message": "success",
  "data": [
    {
      "categoryId": 1,
      "name": "手机数码",
      "icon": "https://example.com/icon1.png",
      "sort": 1
    },
    {
      "categoryId": 2,
      "name": "电脑办公",
      "icon": "https://example.com/icon2.png",
      "sort": 2
    }
  ]
}
```

---

## 4️⃣ 购物车模块

### 4.1 获取购物车列表

**接口地址**：`GET /cart/list`

**是否需要认证**：是

**请求参数**：无

**响应示例**：
```json
{
  "code": 200,
  "message": "success",
  "data": [
    {
      "cartId": 1,
      "productId": 1001,
      "productName": "iPhone 15 Pro",
      "productImage": "https://example.com/iphone.jpg",
      "price": 7999,
      "quantity": 2,
      "selected": true,
      "stock": 100
    }
  ]
}
```

---

### 4.2 添加到购物车

**接口地址**：`POST /cart/add`

**是否需要认证**：是

**请求参数**：
```json
{
  "productId": 1001,  // 必填
  "quantity": 2       // 必填，数量
}
```

**响应示例**：
```json
{
  "code": 200,
  "message": "添加成功",
  "data": {
    "cartId": 1
  }
}
```

**业务逻辑**：
- 如果商品已在购物车，则累加数量
- 检查库存是否充足

---

### 4.3 更新购物车

**接口地址**：`POST /cart/update`

**是否需要认证**：是

**请求参数**：
```json
{
  "cartId": 1,        // 必填
  "quantity": 3,      // 选填，更新数量
  "selected": true    // 选填，是否选中
}
```

**响应示例**：
```json
{
  "code": 200,
  "message": "更新成功",
  "data": null
}
```

---

### 4.4 删除购物车商品

**接口地址**：`DELETE /cart/delete/:id`

**是否需要认证**：是

**请求参数**：
- `id`：购物车ID（路径参数）

**示例**：
```
DELETE /cart/delete/1
```

**响应示例**：
```json
{
  "code": 200,
  "message": "删除成功",
  "data": null
}
```

---

## 5️⃣ 订单模块

### 5.1 创建订单

**接口地址**：`POST /order/create`

**是否需要认证**：是

**请求参数**：
```json
{
  "addressId": 1,           // 必填，收货地址ID
  "cartIds": [1, 2, 3],     // 必填，购物车ID数组
  "remark": "请尽快发货"     // 选填，订单备注
}
```

**响应示例**：
```json
{
  "code": 200,
  "message": "订单创建成功",
  "data": {
    "orderId": 10001,
    "orderNo": "20240101123456789",
    "totalAmount": 15998
  }
}
```

**业务逻辑**：
1. 验证地址是否属于当前用户
2. 验证购物车商品是否属于当前用户
3. 检查商品库存
4. 计算订单总金额
5. 扣减库存
6. 清空已下单的购物车商品

---

### 5.2 获取订单列表

**接口地址**：`GET /order/list`

**是否需要认证**：是

**请求参数**（Query）：
```
status=0           // 订单状态，选填（0-待付款，1-待发货，2-待收货，3-已完成，4-已取消）
page=1            // 页码，默认1
pageSize=10       // 每页数量，默认10
```

**示例**：
```
GET /order/list?status=0&page=1&pageSize=10
```

**响应示例**：
```json
{
  "code": 200,
  "message": "success",
  "data": {
    "list": [
      {
        "orderId": 10001,
        "orderNo": "20240101123456789",
        "status": 0,
        "statusText": "待付款",
        "totalAmount": 15998,
        "products": [
          {
            "productId": 1001,
            "productName": "iPhone 15 Pro",
            "productImage": "https://example.com/iphone.jpg",
            "price": 7999,
            "quantity": 2
          }
        ],
        "address": {
          "name": "张三",
          "phone": "13800138000",
          "detail": "广东省深圳市南山区科技园南区XX路XX号"
        },
        "createdAt": "2024-01-01T12:34:56.000Z"
      }
    ],
    "total": 20,
    "page": 1,
    "pageSize": 10
  }
}
```

---

### 5.3 获取订单详情

**接口地址**：`GET /order/:id`

**是否需要认证**：是

**请求参数**：
- `id`：订单ID（路径参数）

**示例**：
```
GET /order/10001
```

**响应示例**：
```json
{
  "code": 200,
  "message": "success",
  "data": {
    "orderId": 10001,
    "orderNo": "20240101123456789",
    "status": 0,
    "statusText": "待付款",
    "totalAmount": 15998,
    "remark": "请尽快发货",
    "products": [...],
    "address": {...},
    "createdAt": "2024-01-01T12:34:56.000Z",
    "paidAt": null,
    "shippedAt": null,
    "completedAt": null
  }
}
```

---

### 5.4 取消订单

**接口地址**：`POST /order/cancel/:id`

**是否需要认证**：是

**请求参数**：
- `id`：订单ID（路径参数）

**示例**：
```
POST /order/cancel/10001
```

**响应示例**：
```json
{
  "code": 200,
  "message": "订单已取消",
  "data": null
}
```

**业务逻辑**：
- 只有待付款状态的订单可以取消
- 取消后恢复商品库存

---

### 5.5 确认收货

**接口地址**：`POST /order/confirm/:id`

**是否需要认证**：是

**请求参数**：
- `id`：订单ID（路径参数）

**示例**：
```
POST /order/confirm/10001
```

**响应示例**：
```json
{
  "code": 200,
  "message": "确认收货成功",
  "data": null
}
```

**业务逻辑**：
- 只有待收货状态的订单可以确认收货
- 确认后订单状态变为已完成

---

### 5.6 删除订单

**接口地址**：`DELETE /order/delete/:id`

**是否需要认证**：是

**请求参数**：
- `id`：订单ID（路径参数）

**示例**：
```
DELETE /order/delete/10001
```

**响应示例**：
```json
{
  "code": 200,
  "message": "删除成功",
  "data": null
}
```

**业务逻辑**：
- 只有已完成或已取消的订单可以删除

---

## 6️⃣ 地址模块

### 6.1 获取地址列表

**接口地址**：`GET /address/list`

**是否需要认证**：是

**请求参数**：无

**响应示例**：
```json
{
  "code": 200,
  "message": "success",
  "data": [
    {
      "addressId": 1,
      "name": "张三",
      "phone": "13800138000",
      "province": "广东省",
      "city": "深圳市",
      "district": "南山区",
      "detail": "科技园南区XX路XX号",
      "isDefault": true
    }
  ]
}
```

---

### 6.2 添加地址

**接口地址**：`POST /address/add`

**是否需要认证**：是

**请求参数**：
```json
{
  "name": "张三",              // 必填
  "phone": "13800138000",     // 必填
  "province": "广东省",        // 必填
  "city": "深圳市",           // 必填
  "district": "南山区",        // 必填
  "detail": "科技园南区XX路XX号", // 必填
  "isDefault": true           // 选填，是否默认地址
}
```

**响应示例**：
```json
{
  "code": 200,
  "message": "添加成功",
  "data": {
    "addressId": 1
  }
}
```

**业务逻辑**：
- 如果设置为默认地址，将其他地址的 isDefault 设为 false

---

### 6.3 更新地址

**接口地址**：`POST /address/update/:id`

**是否需要认证**：是

**请求参数**：
- `id`：地址ID（路径参数）
- Body：需要更新的字段（同添加地址）

**示例**：
```
POST /address/update/1
```

**请求体**：
```json
{
  "name": "李四",
  "phone": "13900139000"
}
```

**响应示例**：
```json
{
  "code": 200,
  "message": "更新成功",
  "data": null
}
```

---

### 6.4 删除地址

**接口地址**：`DELETE /address/delete/:id`

**是否需要认证**：是

**请求参数**：
- `id`：地址ID（路径参数）

**示例**：
```
DELETE /address/delete/1
```

**响应示例**：
```json
{
  "code": 200,
  "message": "删除成功",
  "data": null
}
```

---

### 6.5 设置默认地址

**接口地址**：`POST /address/setDefault/:id`

**是否需要认证**：是

**请求参数**：
- `id`：地址ID（路径参数）

**示例**：
```
POST /address/setDefault/1
```

**响应示例**：
```json
{
  "code": 200,
  "message": "设置成功",
  "data": null
}
```

---

## 7️⃣ 首页模块

### 7.1 获取首页数据

**接口地址**：`GET /home`

**是否需要认证**：否

**请求参数**：无

**响应示例**：
```json
{
  "code": 200,
  "message": "success",
  "data": {
    "banners": [
      {
        "id": 1,
        "image": "https://example.com/banner1.jpg",
        "link": "/product/1001"
      }
    ],
    "categories": [
      {
        "categoryId": 1,
        "name": "手机数码",
        "icon": "https://example.com/icon1.png"
      }
    ],
    "hotProducts": [
      {
        "productId": 1001,
        "name": "iPhone 15 Pro",
        "price": 7999,
        "image": "https://example.com/iphone.jpg",
        "sales": 520
      }
    ],
    "newProducts": [...]
  }
}
```

---

## 🧪 测试指南

### 使用 Postman 测试

1. **导入 Collection**
   - 导入项目根目录的 `postman_collection.json`

2. **配置环境变量**
   - 变量名：`baseUrl`
   - 本地开发值：`http://localhost:3000`
   - 生产环境值：`https://lyaftwgtwyqr.sealosbja.site`

3. **测试流程**
   ```
   1. 健康检查 → GET /health
   2. 用户注册 → POST /user/register
   3. 用户登录 → POST /user/login（获取token）
   4. 将token保存到环境变量
   5. 测试需要认证的接口
   ```

### 前端 API 封装示例

```javascript
// api/user.js
import request from '@/utils/request'

export const userAPI = {
  // 登录
  login(data) {
    return request.post('/user/login', data)
  },
  
  // 注册
  register(data) {
    return request.post('/user/register', data)
  },
  
  // 获取用户信息
  getUserInfo() {
    return request.get('/user/info')
  },
  
  // 更新用户信息
  updateUserInfo(data) {
    return request.post('/user/update', data)
  }
}

// api/product.js
export const productAPI = {
  // 获取商品列表
  getProductList(params) {
    return request.get('/product/list', { params })
  },
  
  // 获取商品详情
  getProductDetail(id) {
    return request.get(`/product/${id}`)
  },
  
  // 搜索商品
  searchProducts(params) {
    return request.get('/product/search', { params })
  }
}

// api/cart.js
export const cartAPI = {
  // 获取购物车列表
  getCartList() {
    return request.get('/cart/list')
  },
  
  // 添加到购物车
  addToCart(data) {
    return request.post('/cart/add', data)
  },
  
  // 更新购物车
  updateCart(data) {
    return request.post('/cart/update', data)
  },
  
  // 删除购物车商品
  deleteCart(id) {
    return request.delete(`/cart/delete/${id}`)
  }
}
```

---

## 🔍 常见问题

### 1. 跨域问题
后端已配置 CORS，默认允许 `http://localhost:5173`。如果前端端口不同，需要修改后端 `.env` 文件中的 `CORS_ORIGIN`。

### 2. Token 过期处理
Token 有效期为 7 天，过期后会返回 401 状态码。前端需要：
- 监听 401 响应
- 清除本地 token
- 跳转到登录页

### 3. 图片上传
当前版本暂未实现图片上传接口，图片字段使用 URL 字符串。如需上传功能，建议：
- 使用第三方云存储（如阿里云 OSS、七牛云）
- 或在后端添加文件上传接口

### 4. 分页参数
所有列表接口都支持分页：
- `page`：页码，从 1 开始
- `pageSize`：每页数量，建议 10-50
- 响应包含 `total`（总数）和 `totalPages`（总页数）

### 5. 数据库连接
生产环境数据库连接信息：
```
mongodb://root:9bpm8zf4@x-db-mongodb.ns-rpjorlyu.svc:27017
```

---

## 📞 联系方式

如有疑问或需要协助，请联系后端开发团队。

**文档版本**：v1.0  
**更新时间**：2024-12-09  
**维护者**：后端开发团队
